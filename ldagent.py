...
def relaunch(relaunched_ref: list[bool]):
    if not relaunched_ref[0]:
        kill_once()
        relaunched_ref[0] = True

    # wait 5 seconds for emulator shut down
    time.sleep(5)
    ...
    try:
        while True:
            # if relaunched_ref[0]:
            # break
            process_ret = subprocess.run([LDCONSOLE_PATH, "isrunning", '--index', str(EMU_IDX)], capture_output=True,
                                         timeout=10)
            ...
            if process_stdout == 'running':
                ...
                process_ret = subprocess.run(
                    [LDCONSOLE_PATH, 'launchex', '--index', str(EMU_IDX), '--packagename', PACKAGE_NAME],
        ...
        while True:
            process_ret = subprocess.run([ADB_PATH, "-s", f"emulator-{ADB_IDX}", 'shell', 'pidof', PACKAGE_NAME],
                                         capture_output=True, timeout=10)
            logger.debug(f'OIADLYZHXI pidof returncode = {process_ret.returncode}')
            if process_ret.returncode == 0:
                break
            process_ret = subprocess.run(
                [LDCONSOLE_PATH, "runapp", '--index', str(EMU_IDX), '--packagename', PACKAGE_NAME], capture_output=True,
                timeout=10)
...
def recover():
    ...
    try:
        ...
        # force screen size
        process_ret = subprocess.run([LDCONSOLE_PATH, "modify", '--index', str(EMU_IDX), "--resolution", "300,400,120"],
                                     capture_output=True, timeout=10)
...
def kill_once():
    ...
    if process_stdout == 'stop':
        ...
        process_ret = subprocess.run([LDCONSOLE_PATH, 'quit', '--index', str(EMU_IDX)], capture_output=True,
                                     timeout=10)
...
def adb_exec(cmd, timeout=5):
    try:
        process_ret = subprocess.run([ADB_PATH, "-s", f"emulator-{ADB_IDX}"] + cmd, capture_output=True,
                                     timeout=timeout)
        if process_ret.returncode != 0:
